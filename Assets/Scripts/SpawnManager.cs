    ...
    bool StageHasBoss()
{
    ...
    bool TrySpawnPacket(float zPos)
    {
        ...
        SpawnPacketConfig packet = rhythmTable.Pick(stage, _lastPacket);
    ...
    /// hasLeadGap: ilk gruptan once leadGapZ metre bosluk birakilir (DelayedCharger icin).
    ...
    void SpawnFromPacket(SpawnPacketConfig packet, float baseZ)
    {
        ...
        // DelayedCharger: tehditten once player'a bir bosluk birakilir
        float startZ = baseZ + (packet.hasLeadGap ? packet.leadGapZ : 0f);
