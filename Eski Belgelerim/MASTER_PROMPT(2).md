# Top End War — MASTER PROMPT v11
**Repo:** https://github.com/LpeC0/Top-End-War

Projem Unity 6.3 LTS URP 3D mobil runner/ordu oyunu: Top End War

---

## OYUN TANIMI
Runner/auto-shooter + ordu yonetimi.
Player otomatik kosar, surukleme ile X ekseninde hareket eder.
Yolda matematiksel kapılar (sol/sag secim).
Komutan otomatik ates eder; arkasindan formasyon halinde askerler takip eder.
CP = savas gucu + tier kilidi (CAN DEGIL).
Komutan HP ayri bar — 0 olunca GameOver.
1200m boss (anchor modu), sonra haritada yeni bolge ele gecirildi.

**Gelecek vizyon (henuz implemente edilmedi):**
- 3D Dunya haritasi (global, cok dunya)
- Karakter/envanter ekrani (silah, zirh, pet)
- Summon/Chest sistemi
- Challenge mod (3 seritli)
- Level editor, Arena (PvP), Birlik
...
DOTween kurulu
Bullet: OverlapSphere   OnTriggerEnter degil (iki trigger collide etmez)
Enemy tag: "Enemy"      Zorunlu — buyuk E
"Soldier" tag YOK       Unity'de varsayilan degil, ekleme
```
...
SampleScene
  ...
  BossManager         BossManager (HP=41000, biyom carpani aktif)
  ArmyManager         ArmyManager (max 20 asker)
  BiomeManager        BiomeManager (currentBiome: Tas/Orman/Cul/Karli/Tarim)
  ...
  Canvas
      CPText, TierText, PopupText, SynergyText
      DamageFlash (Image, full stretch, alpha=0, RaycastTarget=false)
      PiyadeBar, MekanizeBar, TeknolojiBar (Slider, interactable=false)
      HUDPanel        GameHUD (komutan HP slider + asker sayisi)
```

---

## SCRIPTABLEOBJECT VARLIKLARINI OLUSTURMA (Unity Inspector)
```
Create -> TopEndWar -> Equipment   (silah/zirh)
Create -> TopEndWar -> Pet         (pet)
Create -> TopEndWar -> Gate Data   (kapi tipleri)
Create -> TopEndWar -> Progression Config  (opsiyonel)
```
Player Inspector'da equippedWeapon / equippedArmor / equippedPet slotlarina sur.

---

## KAPI GORSEL SEMASI
```
Yesil      CP artisi         "CP +80", "CP +45"
Parlak yesil  Piyade asker   "PIY x2"
Gri        Mekanik asker     "MEK x2"
Mavi       Teknoloji asker   "TEK x2"
Mor        Merge             "MERGE"
Pembe      Komutan Heal      "KMT +HP"
Acik yesil Asker Heal        "ASK HP+"
Turuncu    PathBoost         "PIY +25%", "MEK +25%"
Sari       Risk              "RISK !"
Altin      Carpan            "CP x1.2"
Kirmizi    Negatif           "CP -60"
```
...
public class EquipmentData : ScriptableObject
{
    ...
    public int baseCPBonus;           // CP'ye duz eklenir
    public float fireRateMultiplier = 1f; // Sadece silah — atisi hizi carpani
}
```
...
public class PetData : ScriptableObject
{
    ...
    public GameObject petPrefab; // Karakterin arkasindan kosacak 3D model
    public Sprite icon;

    [Header("Anchor & Combat Bonuslari")]
    public int cpBonus;
    public float anchorDamageReduction = 0.1f; // Anchor modunda +%10 hasar azaltma
}
```
...
Scriptler (24): ArmyManager, BiomeManager, BossManager, Bullet,
ChunkManager, DifficultyManager, Enemy, EnemyHealthBar, EquipmentData,
GameEvents, GameHUD, GameOverUI, Gate, GateData, GateFeedback,
MorphController, ObjectPooler, PetData, PlayerController, PlayerStats,
ProgressionConfig, SimpleCameraFollow, SoldierUnit, SpawnManager
...
PlayerStats.CP property (field degil) — _baseCP + equipment bonusu.
BossManager HP=41000, biyom carpani aktif.
MorphController: OnBiomeChanged ile shader rengi otomatik guncellenir.
PlayerController: equippedWeapon.fireRateMultiplier ates hizina uygulanir.
```
...
v10     Claude: GPT tahribati temizlendi, ordu sistemi, DESIGN_BIBLE v3
        + Gemini: EquipmentData, PetData, _baseCP mimarisi eklendi
v11     Claude: BossManager HP=41k+biyom carpani, MorphController shader
        wiring (OnBiomeChanged), PlayerController equipment fire rate,
        SoldierUnit tag hatasi duzeltildi, GameHUD slider fill rect fix,
        SpawnManager kapi isimleri netlestirildi, DESIGN_BIBLE v3.1
        (Path sistemi vizyon notu eklendi)
```
